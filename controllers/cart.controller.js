...
/* Helper: Finds user cart; if not found, creates a new empty cart */
async function getCartForUser(userid) {
    let cart = await CartModel.findOne({ user: userid }); // Find cart by user
    if (!cart) {
        cart = await CartModel.create({ user: userid, items: [] }); // Create empty cart if not found
    }
    return cart;
}
...
export async function addToCart(req, res) {
    try {
        const { productId, quantity = 1 } = req.body; // Product ID & quantity
